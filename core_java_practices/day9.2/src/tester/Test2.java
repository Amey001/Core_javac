...
public class Test2 {
	...
	public static void main(String[] args) {
		...
		// display list of products : using forEach
				...
				boolean removed
				=productList.removeIf(p -> p.getProductCategory().equals(chosenCategory));
			...
			//we are using two higher order methods ::
...
//			}
			...
			System.out.println("product list before :: ");
			productlist.forEach(p -> System.out.println(p));
			
			System.out.println("enter the category :: ");
			Category cat=Category.valueOf(sc.next().toUpperCase());
			
			//we will not use foreach lamda expression here 
			System.out.println(productlist.removeIf(p -> p.getProductCategory()==cat));
			
			System.out.println("product list after :: ");
			productlist.forEach(p-> System.out.println(p));
