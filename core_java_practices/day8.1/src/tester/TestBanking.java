...
//						float interest;
//						for (BankAccount a : acctMap.values()) {
//							if (a.getType() == AcctType.SAVING) {
//								System.out.println(a);
//								int month = Period.between(a.getCreatedOn(), LocalDate.now()).getMonths();
//								if(month<3)
//									interest=3.5f;
//								if(month>3 && month<6)
//									interest=6.2f;
//								else
//									interest=9.0f;
//								a.interestCalculate(interest, month);
//							}
//						}
						
						System.out.println("enter interest rate :: ");
						float interest=sc.nextFloat();
						for(BankAccount b: acctMap.values())
						{
							if(b.getType()==AcctType.SAVING)
							{
								int year=Period.between(b.getCreatedOn(), LocalDate.now()).getYears();
								b.interestCalculate(interest, year);
							}
						}
